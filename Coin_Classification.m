...
for imagenum = 1 : N
	...
	thresholdLevel = minGrayLevel_Red + 0.06*(maxGrayLevel_Red  - minGrayLevel_Red);
	binaryImage_Red = redChannel  > thresholdLevel;

	thresholdLevel = minGrayLevel_Green + 0.03*(maxGrayLevel_Green  - minGrayLevel_Green);
	...
	for classnum=1:maxclasses
		shape=getlargest(binary_image_thresh,1);
		figure;
		imagesc(shape);
		trueclasses(classnum) = input(['select 1-£1,2-£2,3-50p,4-5p,5-WashSmallHole,6-WashBigHole,7-Battery,8-20p,9-RightAngle,10-nut, then press enter','true class (1..',int2str(maxclasses),')\n?']);
...
eval(['load ','C:\Users\ballanr\Desktop\Data_Science\Edinburgh_University\Vision_Robotics\Vision_Assignment\StatsModel\model10',' maxclasses Means Invcors Aprioris']);
...
while ~(run == 0)
	...
	for classnum=1:maxclasses
		...
		figure;
		imagesc(shape);
		vec = getproperties(shape);
		%class=classify(vec,maxclasses,Means,Invcors,Dim,Aprioris)
		class=classify(vec,Vec,Classes) %Following (SAMPLE,TRAINING,GROUP) format
